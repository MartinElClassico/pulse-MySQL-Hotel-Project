...
-- @block
CREATE TABLE huvud_gäst(
    huvud_gäst_id INT PRIMARY KEY AUTO_INCREMENT,
    förnamn VARCHAR(255),
    efternamn VARCHAR(255),
...
CREATE TABLE försäljning(
    försäljning_id INT PRIMARY KEY AUTO_INCREMENT,
    rum_id INT,
    personal_id INT,
    faktura_id INT,
    summa DECIMAL(8,2),
    datum TIMESTAMP,
    CONSTRAINT försäljning_fk_rum FOREIGN KEY (rum_id) REFERENCES rum(rum_id),
    CONSTRAINT försäljning_fk_personal FOREIGN KEY (personal_id) REFERENCES personal(personal_id),
    CONSTRAINT försäljning_fk_faktura FOREIGN KEY (faktura_id) REFERENCES faktura(faktura_id)
);
-- @block
CREATE TABLE bokning(
    ...
    CONSTRAINT bokning_fk_huvud_gäst FOREIGN KEY (huvud_gäst_id) REFERENCES huvud_gäst(huvud_gäst_id),
